  ...
  return (
    <Sidebar className="border-r border-sidebar-border">
      ...
      <SidebarContent className="px-2 py-4">
        {!collapsed && profile && (
          <div className="mx-2 mb-4 p-3 rounded-lg bg-primary/10 border border-primary/20">
            <div className="flex items-center gap-2 mb-2">
              <Badge variant="outline" className="text-xs">Executivo</Badge>
            </div>
        ...
        <SidebarGroup>
          <SidebarGroupLabel className="text-xs text-muted-foreground uppercase tracking-wider mb-2">Menu</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {menuItems.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild isActive={isActive} tooltip={item.title}>
                      <NavLink to={item.url} className="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors" activeClassName="bg-primary/10 text-primary">
      ...
      <SidebarFooter className="border-t border-sidebar-border p-4">
        <DropdownMenu>
            ...
            <Button variant="ghost" className="w-full justify-start gap-3 px-3 py-2 h-auto">
              ...
              {!collapsed && (
                <div className="flex flex-col items-start text-left">
                  <span className="text-sm font-medium truncate max-w-[120px]">{profile?.name || profile?.email || 'Executivo'}</span>
          ...
          <DropdownMenuContent align="start" className="w-56">
            <DropdownMenuItem disabled><User className="mr-2 h-4 w-4" />Meu Perfil</DropdownMenuItem>
            <DropdownMenuSeparator />
...
export default function ExecutiveLayout() {
  ...
  useEffect(() => {
    ...
    // This layout is deprecated — redirect to modular routes
    if (!isLoading && isAuthenticated) {
