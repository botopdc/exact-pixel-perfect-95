...
import { useExternalTopArticles, useExternalBestRatedArticles, useExternalRecentArticles } from '@/hooks/useExternalArticles';
import { TrendingUp, ThumbsUp, Clock, ArrowRight } from 'lucide-react';
import { Skeleton } from '@/components/ui/skeleton';

function ArticleListItem({ article }: { article: { id: number; title: string } }) {
  return (
    <Link
...
export function ExternalArticleSidebar() {
  const { data: topArticles, isLoading: topLoading } = useExternalTopArticles();
  const { data: bestRatedArticles, isLoading: ratedLoading } = useExternalBestRatedArticles();
  const { data: recentArticles, isLoading: recentLoading } = useExternalRecentArticles();
